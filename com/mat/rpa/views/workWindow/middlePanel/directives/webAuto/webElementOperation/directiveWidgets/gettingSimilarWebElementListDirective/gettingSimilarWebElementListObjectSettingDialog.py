...
class GettingSimilarWebElementListObjectSettingDialog(flowSettingDialog.BasicFlowSettingDialog):
    def __init__(self, title, directive_id, id, parent=None):
        ...
        #参照修改infolabel的提示信息
        self.setInfoLabelText("可通过捕获元素、CSS、XPath三种方式定位到网页上的一组相似元素，获取对象或文本等信息")
    ...
    def regularTabUI(self):
        ...
        self.elementOperationLabel = QLabel()
        self.elementOperationLabel.setFont(__font)
        self.elementOperationLabel.setText("元素操作:")
        self.directiveInputSettingLayout.addWidget(self.elementOperationLabel, 5, 0, 1, 1, Qt.AlignRight)
        self.elementOperationCombobox = QComboBox()
        self.elementOperationCombobox.setObjectName("browserTypeCombobox")
        self.elementOperationCombobox.setFont(__font)
        self.elementOperationCombobox.setMinimumSize(600, 50)
        self.elementOperationCombobox.addItems(["获取元素对象", "获取元素文本内容","获取元素值","获取元素链接地址","获取元素源代码","获取元素属性"])
        self.elementOperationCombobox.setItemDelegate(QStyledItemDelegate())
        self.elementOperationCombobox.setStyleSheet(
            "QComboBox QAbstractItemView{selection-background-color: #f0f0f0;selection-color: #000;} QComboBox QAbstractItemView::item{min-height: 50px;}")
        self.directiveInputSettingLayout.addWidget(self.elementOperationCombobox, 5, 1, 1, 3, Qt.AlignCenter)
        self.elementOperationTipLabel = flowSettingDialog.createTipLabel("元素操作",
                                                                     "  获取元素的信息")
        self.directiveInputSettingLayout.addWidget(self.elementOperationTipLabel, 5, 4, 1, 1, Qt.AlignLeft)

        self.tinyLayout = QLayout
        self.outputSimilarElementNumCheckbox = QCheckBox()
        self.outputSimilarElementNumCheckbox.setText("输出相似元素个数")
        self.outputSimilarElementNumCheckbox.setFont(__font)
        self.outputSimilarElementNumCheckbox.setChecked(False)
        self.outputSimilarElementNumCheckbox.stateChanged.connect(self.changeOutputUI)
        self.directiveInputSettingLayout.addWidget(self.outputSimilarElementNumCheckbox, 6, 1, 1, 1, Qt.AlignCenter)
        self.outputSimilarElementNumTipLabel = flowSettingDialog.createTipLabel("输出相似元素个数", "  是否输出相似元素个数")
        self.directiveInputSettingLayout.addWidget(self.outputSimilarElementNumTipLabel, 6, 2, 1, 1, Qt.AlignLeft)
        ...
        self.saveElementObjectLabel.setText("保存相似元素列表至:")
        ...
        self.saveElementObjectTipLabel = flowSettingDialog.createTipLabel("保存相似元素列表至",
                                                                          "  指定一个变量名称,该变量用于保存获\n  取到的相似元素列表")

        self.saveElementNumLabel = QLabel()
        self.saveElementNumLabel.setFont(__font)
        self.saveElementNumLabel.setText("保存相似元素个数至:")
        self.directiveOutputSettingLayout.addWidget(self.saveElementNumLabel, 1, 0, 1, 1, Qt.AlignLeft)
        self.saveElementNumCombobox = QComboBox()
        self.saveElementNumCombobox.setFont(__font)
        self.saveElementNumCombobox.setMinimumSize(600, 50)
        self.saveElementNumCombobox.setEditable(True)  # 设置可以编辑
        self.directiveOutputSettingLayout.addWidget(self.saveElementNumCombobox, 1, 1, 1, 3, Qt.AlignLeft)
        self.saveElementNumTipLabel = flowSettingDialog.createTipLabel("保存相似元素个数",
                                                                          "  指定一个变量名称,该变量用于保存获\n  取到的相似元素个数")

        self.directiveOutputSettingLayout.addWidget(self.saveElementNumTipLabel, 1, 4, 1, 1, Qt.AlignLeft)

        self.fatherElementLabel.hide()
        self.fatherElementCombobox.hide()
        self.fatherElementTipLabel.hide()
        self.saveElementNumLabel.hide()
        self.saveElementNumCombobox.hide()
        self.saveElementNumTipLabel.hide()
    ...
    def changeOutputUI(self):
        if self.outputSimilarElementNumCheckbox.isChecked() is True:
            self.saveElementNumLabel.show()
            self.saveElementNumCombobox.show()
            self.saveElementNumTipLabel.show()
        else:
            self.saveElementNumLabel.hide()
            self.saveElementNumCombobox.hide()
            self.saveElementNumTipLabel.hide()
    ...
    def executeStep(self):
        try:
            ...
            if self.relatedTypeCombobox.currentIndex() == 0:
                fatherWebElement = client.find_element(By.XPATH,
                ...
                if self.getChildMathodCombobox.currentIndex() == 1:
                    ...
                    childWebElement = client.find_element(By.XPATH,
                                                          self.elementDaoMongo.getOneElement(self.elementLabel.text())[
                                                              "xpath"] + '/*[%s]' % (positon))
                else:
                    childWebElements = client.find_element(By.XPATH,
                                                           self.elementDaoMongo.getOneElement(self.elementLabel.text())[
                                                               "xpath"] + '/*')
            elif self.relatedTypeCombobox.currentIndex() == 2:
                if self.whichNeighbourCombobox.currentIndex() == 0:
                    last_neighbourWebElement = client.find_element(By.XPATH,
                                                                   self.elementDaoMongo.getOneElement(
                                                                       self.elementLabel.text())[
                                                                       "xpath"] + '/preceding-sibling::*')
                if self.whichNeighbourCombobox.currentIndex() == 1:
                    next_neighbourWebElement = client.find_element(By.XPATH,
                                                                   self.elementDaoMongo.getOneElement(
                                                                       self.elementLabel.text())[
                                                                       "xpath"] + '/following::*')
