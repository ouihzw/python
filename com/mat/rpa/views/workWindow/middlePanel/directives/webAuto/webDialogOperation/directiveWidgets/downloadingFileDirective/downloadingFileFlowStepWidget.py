...
from . import downloadingFileObjectSettingDialog
from ...webDialogContants import WebDialogConstants


class DownloadingFileFlowWidget(flowStepWidget.FlowStepWidget):
    def __init__(self, scene, title="Undefined Node", type=" Node ", id=None, inputs=[], outputs=[]):
        super(DownloadingFileFlowWidget, self).__init__(scene, title, type, id, inputs=[1, 2, 3, 4], outputs=[])
        self.directive["command"] = WebDialogConstants.downloadingFileDirective
        ...
        self.dialog = downloadingFileObjectSettingDialog.DownloadingFileObjectSettingDialog(self.title,
                                                                                                self.directive["_id"],
                                                                                                self.id)
        ...
        if data["screenshot_area"] == 0:
            if data["save_location"] == True:
                ...
                if data["name"] == True:
                    ...
                    super().updateSecondLineInfo(("${" + data["web_object"] + "}", data["element_name"],
                                                  data["save_location_name"], "${" + data["output"] + "}"))
                else:
                    self.infoFormat = u"在网页%s中对%s元素进行截图，将结果保存到文件夹%s中，使用自定义文件名%s，将存图路径保存到%s"
                    super().updateSecondLineInfo(("${" + data["web_object"] + "}", data["element_name"],
                                                  data["save_location_name"], data["custom_name"],
                                                  "${" + data["output"] + "}",))
            ...
            if data["save_location"] == True:
                ...
                if data["name"] == True:
                    ...
                    super().updateSecondLineInfo(
                        ("${" + data["web_object"] + "}", data["save_location_name"], "${" + data["output"] + "}"))
                else:
                    self.infoFormat = u"对网页%s可视区域进行截图，将结果保存到文件夹%s中，使用自定义文件名%s，将存图路径保存到%s"
                    super().updateSecondLineInfo(("${" + data["web_object"] + "}", data["save_location_name"],
                                                  data["custom_name"], "${" + data["output"] + "}"))
        else:
            if data["save_location"] == True:
