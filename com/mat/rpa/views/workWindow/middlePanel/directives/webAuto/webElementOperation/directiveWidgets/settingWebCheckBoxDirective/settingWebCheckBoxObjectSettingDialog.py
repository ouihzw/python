...
class SettingWebCheckBoxObjectSettingDialog(flowSettingDialog.BasicFlowSettingDialog):
    flowTextList = pyqtSignal(list)
    def __init__(self, title, directive_id, id, parent=None):
        super(SettingWebCheckBoxObjectSettingDialog, self).__init__(title, directive_id, id, parent)
        self.directiveDaoObj = DirectiveDao()
        self.webSave = webAutoSave.WebAutoSave()
        self.elementDaoMongo = elementDao.ElementDao()
        self.setInfoLabelText(u"将网页中的复选框的状态设置为勾选或不勾选")
        __font = QFont()
    ...
    def regularTabUI(self):
        ...
        self.objectComboBox.addItems(["勾选", "取消勾选", "反选"])
    ...
    def executeStep(self):
        try:
            ...
            if self.objectComboBox.currentIndex() == 0:
                if webElement.is_selected() is True:
                    return
                else:
                    webElement.click()
            elif self.objectComboBox.currentIndex() == 1:
                if webElement.is_selected() is True:
                    webElement.click()
                else:
                    return
            elif self.objectComboBox.currentIndex() == 2:
                webElement.click()
